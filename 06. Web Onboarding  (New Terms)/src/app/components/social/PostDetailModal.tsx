...
// Instagram/LinkedIn-style side-by-side post detail modal
...
export function PostDetailModal({ post, isOpen, onClose }: PostDetailModalProps) {
  ...
  const isVideoPost = post.type === 'video' || post.videoUrl;
  const isPollPost = post.type === 'poll' || post.poll;
  const isLinkPost = post.type === 'link' || post.link;

  // Transform poll data for display
  const pollOptions = isPollPost && post.poll ? post.poll.options.map((opt: any) => ({
    text: opt.text,
    percentage: opt.percentage,
    votes: opt.votes,
  })) : [];

  const totalVotes = isPollPost && post.poll ? post.poll.totalVotes : 0;
  const pollEndsIn = isPollPost && post.poll ? post.poll.endsIn : '';

  // Transform link data for display
  const linkPreview = isLinkPost && post.link ? {
    url: post.link.url,
    title: post.link.title,
    description: post.link.description,
    image: post.link.thumbnail,
    domain: post.link.domain,
  } : null;
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <motion.div
          ...
          className="relative w-full max-w-6xl h-[90vh] mx-4 flex rounded-2xl overflow-hidden shadow-2xl z-10"
        >
          {/* Close Button */}
          <button
            onClick={onClose}
          ...
          <div className={`w-[60%] flex items-center justify-center ${isDark ? 'bg-black' : 'bg-black'}`}>
            {/* Video */}
            {isVideoPost && (
            ...
            {/* Poll - displayed on left when no image/video */}
            {isPollPost && !post.image && !isVideoPost && (
              <div className={`w-full h-full overflow-y-auto p-8 ${isDark ? 'bg-[#1a1d29]' : 'bg-white'}`}>
                <div className="space-y-3">
                  ...
                  {pollOptions.map((option: any, index: number) => (
                    <div
                      ...
                      className={`relative p-4 rounded-xl border cursor-pointer transition-all ${
                        isDark
                          ? 'border-[#363b4e] hover:border-purple-500/50 bg-[#242833]'
                          : 'border-slate-200 hover:border-purple-500/50 bg-slate-50'
                      }`}
                    >
                      ...
                      <div className={`h-2 rounded-full overflow-hidden ${isDark ? 'bg-[#1a1d29]' : 'bg-slate-200'}`}>
                        <div
                  ...
                  <p className={`text-sm text-center pt-4 ${isDark ? 'text-slate-500' : 'text-slate-500'}`}>
                    {totalVotes?.toLocaleString()} total votes • {pollEndsIn}
                  </p>
                </div>
              </div>
            )}

            {/* Link Preview - displayed on left when present */}
            {linkPreview && !post.image && !isVideoPost && !isPollPost && (
              <div className={`w-full h-full overflow-y-auto p-8 ${isDark ? 'bg-[#1a1d29]' : 'bg-white'}`}>
                <a
                  ...
                  className={`block border rounded-xl overflow-hidden transition-all hover:shadow-lg ${
                    isDark ? 'border-[#363b4e] bg-[#242833]' : 'border-slate-200 bg-slate-50'
                  }`}
                >
                  {linkPreview.image && (
                    <img
            ...
            {/* Text-only post */}
            {!post.image && !isVideoPost && !isPollPost && !linkPreview && (
              <div className={`w-full h-full flex items-center justify-center p-12 ${isDark ? 'bg-[#1a1d29]' : 'bg-white'}`}>
                <p className={`text-2xl text-center leading-relaxed ${isDark ? 'text-slate-300' : 'text-slate-800'}`}>
                  {post.content}
                </p>
              </div>
            )}
          ...
          <div className={`w-[40%] flex flex-col ${isDark ? 'bg-[#1a1d29]' : 'bg-white'}`}>
            {/* Post Header */}
            <div className={`p-5 border-b ${isDark ? 'border-[#363b4e]' : 'border-slate-200'}`}>
              ...
              {post.content && (
                <div className="mt-4">
                  <p className={`text-sm ${isDark ? 'text-slate-300' : 'text-slate-800'}`}>
                    {post.content}
                  </p>
                </div>
              )}
            ...
            {/* Stats */}
            <div className={`px-5 py-3 border-b ${isDark ? 'border-[#363b4e]' : 'border-slate-200'}`}>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
              {comments.map((c) => (
                <div key={c.id} className="flex gap-3">
                  <div className="w-8 h-8 rounded-full bg-gradient-to-br from-purple-500 to-orange-500 flex items-center justify-center text-xs shrink-0 overflow-hidden">
            ...
            <div className={`p-4 border-t ${isDark ? 'border-[#363b4e]' : 'border-slate-200'}`}>
              <div className="flex gap-2 items-center">
                <div className="w-8 h-8 rounded-full bg-gradient-to-br from-purple-500 to-orange-500 flex items-center justify-center text-xs shrink-0">
