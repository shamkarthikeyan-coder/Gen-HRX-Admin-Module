...
import { ThumbsUp, MessageCircle, Share2, Check, ExternalLink, Bookmark } from 'lucide-react';
import { ChatCircleDots, CheckFat, HandsClapping, SketchLogo, TrendUp, Megaphone } from '@phosphor-icons/react';
...
export function LinkPostCard({ post, onPostClick, onSave }: LinkPostCardProps) {
    ...
    <div className={`${isDark ? 'bg-[#242833]' : 'bg-white'} px-6 py-6 relative`}>
      ...
      <div className="flex items-start gap-3 mb-4">
        <div className="w-12 h-12 rounded-full bg-gradient-to-br from-purple-500 to-orange-500 flex items-center justify-center text-lg shrink-0 overflow-hidden">
          <img src="https://images.unsplash.com/photo-1655249493799-9cee4fe983bb?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxwcm9mZXNzaW9uYWwlMjBidXNpbmVzcyUyMHBlcnNvbiUyMGhlYWRzaG90fGVufDF8fHx8MTc3MzA5MDQwN3ww&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral" alt={post.author} className="w-full h-full object-cover" />
        </div>
        ...
        {/* Parley Tag and Three Dots Menu */}
        <div className="flex items-center gap-2">
          {/* Parley Tag */}
          <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full ${
            isDark ? 'bg-[#363b4e]' : 'bg-slate-100'
          } shrink-0`}>
            <ChatCircleDots className="w-4 h-4 text-blue-500" weight="fill" />
      ...
      <div className="mb-4">
        <a
          href={`https://${post.link.url}`}
          target="_blank"
          rel="noopener noreferrer"
          className={`block rounded-xl border overflow-hidden transition-all hover:shadow-lg ${
            isDark ? 'border-[#363b4e] bg-[#1a1d29]' : 'border-slate-200 bg-slate-50'
          }`}
        >
          {/* Thumbnail */}
          ...
          <div className="p-4">
            <div className={`text-xs mb-2 flex items-center gap-1 ${
              ...
              <span>{post.link.domain || post.link.url.split('/')[2]}</span>
            </div>
            <h4 className={`text-sm font-semibold mb-2 line-clamp-2 ${isDark ? 'text-white' : 'text-slate-900'}`}>
              {post.link.title}
            </h4>
